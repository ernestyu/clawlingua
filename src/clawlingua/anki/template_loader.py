...
def _normalize_legacy_template(data: dict) -> dict:
    ...
    if "model_name" not in normalized:
        normalized["model_name"] = normalized.get("note_type_name_default", "ClawLingua Cloze")
    if "deck_name" not in normalized:
        normalized["deck_name"] = normalized.get("deck_name_default", "ClawLingua Default Deck")
...
def load_anki_template(path: Path) -> AnkiTemplateSpec:
    if not path.exists():
        raise build_error(
            ...
            cause="Anki 模板文件不存在。",
            detail=f"path={path}",
            next_steps=["检查 CLAWLINGUA_ANKI_TEMPLATE 路径"],
    ...
    try:
        ...
        raise build_error(
            ...
            detail=str(exc),
            next_steps=["修复模板字段", "重新运行 clawlingua doctor 检查模板"],
            exit_code=ExitCode.SCHEMA_ERROR,
        ) from exc

...
from .pipeline.build_lingua_deck import BuildDeckOptions, run_build_lingua_deck
from .pipeline.build_textbook_deck import BuildTextbookDeckOptions, run_build_textbook_deck
...
lingua_app = typer.Typer(no_args_is_help=True, help="Lingua pipelines.")
textbook_app = typer.Typer(no_args_is_help=True, help="Textbook pipelines.")
lingua_build_app = typer.Typer(no_args_is_help=True, help="Build commands.")
textbook_build_app = typer.Typer(no_args_is_help=True, help="Build commands.")
...
def _echo_error(error: ClawLearnError) -> None:
...
def doctor(
    ...
    def _impl() -> None:
        ...
        # small LLM config and connectivity checks (CLAWLEARN_TRANSLATE_LLM_*)
        if cfg.translate_llm_base_url or cfg.translate_llm_model or cfg.translate_llm_api_key:
            ...
            if not cfg.translate_llm_base_url:
                missing_fields.append("CLAWLEARN_TRANSLATE_LLM_BASE_URL")
            if not cfg.translate_llm_model:
                missing_fields.append("CLAWLEARN_TRANSLATE_LLM_MODEL")
            if not (cfg.translate_llm_api_key or cfg.llm_api_key):
                ...
                missing_fields.append("CLAWLEARN_TRANSLATE_LLM_API_KEY or CLAWLEARN_LLM_API_KEY")
...
@lingua_build_app.command("deck", help=BUILD_DECK_HELP)
def lingua_build_deck(
    ...
    prompt_lang: str | None = typer.Option(
        None,
        "--prompt-lang",
        help="Prompt language for multi-lingual prompts (en|zh). Overrides CLAWLEARN_PROMPT_LANG.",
    ),
...
@textbook_build_app.command("deck", help=BUILD_DECK_HELP)
def textbook_build_deck(
    input_value: str = typer.Argument(..., help="Path to .txt/.md/.epub input."),
    source_lang: str | None = typer.Option(None, "--source-lang", help="Source language code."),
    target_lang: str | None = typer.Option(None, "--target-lang", help="Target language code."),
    input_char_limit: int | None = typer.Option(
...
lingua_app.add_typer(lingua_build_app, name="build")
textbook_app.add_typer(textbook_build_app, name="build")
app.add_typer(lingua_app, name="lingua")

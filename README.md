...
- `CLAWLEARN_EXTRACT_PROMPT` / `CLAWLEARN_EXPLAIN_PROMPT` can pin default
...
ClawLearn uses `edge_tts` to synthesize audio for the `Original` sentence of
...
- At runtime, the language variant is selected based on
  `CLAWLEARN_PROMPT_LANG` (or the `--prompt-lang` CLI override).
...
For users who prefer a browser-based interface, ClawLearn ships an optional
...
- **Run** - upload a `.txt`/`.md`/`.epub` file, select source/target language,
  ...
  `CLAWLEARN_OUTPUT_DIR/<run_id>` and the final deck to
  `CLAWLEARN_EXPORT_DIR/<run_id>/output.apkg`.
- **Config** - a `.env` editor for common `CLAWLEARN_*` settings (LLM
  ...
  it via `clawlearn.config.validate_base_config` + `validate_runtime_config`,
  ...
  `CLAWLEARN_EXTRACT_PROMPT` / `CLAWLEARN_EXPLAIN_PROMPT` dropdowns.
